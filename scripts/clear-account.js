 ...
 *
 * Usage (from repo root via Railway):
 *   railway run node server/scripts/clear-account.js dov@doveandbearinc.com
 *
 * Or locally from server/ dir:
 *   cd server && node scripts/clear-account.js dov@doveandbearinc.com
 */
...
const dbPath = process.env.DB_PATH || path.join(__dirname, '../server/data/constructionbear.db');
